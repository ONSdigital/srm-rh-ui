...
workers = os.getenv('GUNICORN_WORKERS') if os.getenv('GUNICORN_WORKERS') else multiprocessing.cpu_count() * 2 + 1
worker_class = os.getenv('GUNICORN_WORKER_CLASS', 'sync')

# Configure the access logs to be GCP compatible JSON structure
access_log_format = ('{"severity":"DEBUG","method":"%(m)s","url":"%(U)s","remote_address":"%(h)s","date":"%(t)s",'
...
print(
    ...
    f'"service":"rh_ui", "event":"Booting gunicorn with {workers} workers"}}'
)
