...
def test_en_enter_uac_success(test_client):
    with patch(RH_SERVICE_GET_PATH) as mock_get:
        ...
        response = test_client.post(EN_START_ROUTE, data={"access-code": "1234567890123456"})
...
def test_en_enter_uac_blank(test_client):
    response = test_client.post(EN_START_ROUTE, data={"access-code": ""}, follow_redirects=True)
...
def test_en_enter_uac_invalid_length(test_client):
    response = test_client.post(EN_START_ROUTE, data={"access-code": "testing"}, follow_redirects=True)
...
def test_en_enter_uac_inactive(test_client):
    ...
    with patch(RH_SERVICE_GET_PATH) as mock_get:
        ...
        response = test_client.post(EN_START_ROUTE, data={"access-code": "1234123412341234"})
...
def test_en_enter_uac_not_found(test_client):
    ...
    with patch(RH_SERVICE_GET_PATH) as mock_get:
        ...
        response = test_client.post(EN_START_ROUTE, data={"access-code": "1234567890123456"}, follow_redirects=True)
